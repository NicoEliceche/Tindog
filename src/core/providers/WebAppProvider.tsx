...
import { requestCoarseLocation } from '@core/geo/browserLocation';
...
interface WebAppValue {
  ...
  /** Zona aproximada del usuario: null mientras no dio permiso. */
  userZone: LatLng | null;
  /** Si dijo que no: sirve para explicar por que no hay filtro por radio. */
  locationDenied: boolean;
  /** Pide la ubicacion. Devuelve la zona o null si no se pudo. */
...
export function WebAppProvider({ children }: { children: React.ReactNode }) {
  ...
  /**
   * Zona aproximada de quien usa la aplicacion.
   *
   * Se pide una vez al arrancar y se guarda ya redondeada. Sin esto, la
   * distancia a cada mascota no se puede saber y el filtro por radio no
  ...
  const requestLocation = useCallback(async () => {
    const { zone, failure } = await requestCoarseLocation();
    if (zone) { setUserZone(zone); setLocationDenied(false); return zone; }
    setLocationDenied(failure === 'denied');
    return null;
  }, []);
  ...
  const [discoveryPets, setDiscoveryPets] = useState(pets);

  /**
   * Las mascotas con su distancia hasta quien mira.
   *
   * Sale de comparar dos zonas aproximadas, asi que es una cuenta y no un
   * dato guardado. Sin ubicacion del usuario, `distanceKm` queda sin definir
   * y el filtro por radio deja pasar a todas: es preferible mostrar de mas
   * que esconder por un permiso que nadie dio.
   */
  const discoveryPetsWithDistance = useMemo(() => (
    discoveryPets.map((pet) => (
      userZone && pet.zone
        ? { ...pet, distanceKm: distanceKm(userZone, pet.zone) }
        : { ...pet, distanceKm: undefined }
    ))
  ), [discoveryPets, userZone]);

  const [requests, setRequests] = useState<WebConnectionRequest[]>([
