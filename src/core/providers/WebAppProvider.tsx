...
export function WebAppProvider({ children }: { children: React.ReactNode }) {
  ...
  const value: WebAppValue = {
    ...
    profile, updateProfile: (value) => setProfile((current) => ({ ...current, ...value })), discoveryPets, dismissPet: (id) => setDiscoveryPets((current) => current.filter((item) => item.id !== id)), resetDiscovery: () => setDiscoveryPets(pets), requests,
