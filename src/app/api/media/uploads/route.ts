...
import { createPresignedImageUpload } from '@core/security/objectStorage';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const body = await request.json() as { purpose?: unknown; petId?: unknown; mime?: unknown; size?: unknown; checksumSha256?: unknown };
    ...
    if ((purpose !== 'profile_avatar' && purpose !== 'pet_photo') || !ALLOWED_IMAGE_MIMES.has(mime) || !Number.isInteger(size) || size < 1 || size > MAX_IMAGE_BYTES || !/^[A-Za-z0-9+/]{43}=$/.test(checksumSha256)) return withAuthCors(NextResponse.json({ error: 'Invalid upload request' }, { status: 400 }), request);
    ...
    const uploadUrl = await createPresignedImageUpload({ key: quarantineKey, mime, size, checksumSha256 });
