...
export function ProfileScreen({ onLogout }: { onLogout: () => Promise<void> }) {
  ...
  return <View style={styles.screen}>
    {/* El encabezado queda fijo y solo baja el contenido: la tuerca de
        configuracion tiene que estar siempre al alcance. */}
    <View style={[styles.header, { paddingTop: Math.max(insets.top + 16, 24) }]}>
      <GoldHeading style={styles.title}>Perfil</GoldHeading>
      <Pressable accessibilityRole="button" accessibilityLabel="Abrir configuración" onPress={() => navigation.navigate('Settings')} style={styles.settings}>
        <Ionicons name="settings-outline" size={23} color={theme.colors.primary} />
      </Pressable>
    </View>
    ...
    <Pressable accessibilityRole="button" onPress={() => navigation.navigate('Safety')} style={styles.row}><Ionicons name="shield-checkmark-outline" size={22} color={theme.colors.primary} /><View style={{ flex: 1 }}><Text style={styles.rowTitle}>Seguridad</Text><Text style={styles.rowDetail}>Bloqueos y pautas para los encuentros</Text></View><Ionicons name="chevron-forward" size={19} color={theme.colors.textMuted} /></Pressable>
